...
Cone-beam CT (CBCT) is a common imaging modality employed during Radiofrequency Ablation procedures. During the ablation, generating a thermal map would be quite useful.
...
\item Take the ROI, filter the ROI in the baseline and comparison scans with an averaging filter, and calculate the difference
\item Use the result to locate where the ablation likely occurred
\item Calculate the Spatial Offset RMSE value for the ROI
\item Correlate Spatial Offset RMSE values from inside the temperature zones with the measured temperature data at the needle and thermocouple tips. 
